...
#Larry Sanders
def B_Search(Item, List):
	MidPoint = len(List)/2# assigns midpoint
	 
	if Item == MidPoint: #If the Item is the Midpoint on the firt Search It will the return the index of the Item.
		return  Item-1 #
	if Item > MidPoint: # If it is higher than the NidPoint.
		Midpoint= len(Half2)/2 # This will send the search area to the greater half of the list and give it anpther midpoint.
		if Item == Midpoint: # If it is equal to the new Midpoint it will be returned.
			return Item - 1 #
		elif Item != Midpoint: #If it is not it will send it to another search.
			Midpoint = Midpoint / 2 ##This part Narrows the searh by spliting the half in two pieces.
			if Item == Midpoint: #If it is equal it will be returned.
				return Item -1 #
	if Item < MidPoint: # if the Item is less than the midpoint this scope will take effect.
		MIDPOINT=len(Half1)/2#
		if Item == MIDPOINT:#
			return  Item-1#
		elif Item != MIDPOINT:#
			MIDPOINT = MIDPOINT / 2#
			if Item != MIDPOINT:#
				MIDPOINT = MIDPOINT /2#
				return Item -1# This follows the same algorithim as the revious scope.
	else:
		return -1 # If the Item isn't found it will return -1.
